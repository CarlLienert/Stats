...
---independent and identically distributed - Identically Distributed means that there are no overall trendsâ€“the distribution doesn't fluctuate and all items in the sample  are taken 
from the same probability distribution. Independent means that the sample    items are all independent events. In other words, they aren't connected to each other  in any way.
...
---sampling without replacement - oppisite of replacement
...
Scatter ex: plot(Spruce$Ht.change, Spruce$Di.change, main="Height Change vs. Diameter Change",xlab="Height Change ", ylab="Diameter Change ", pch=19)
...
** Sampling with replacement (N=5, n=2 there are 5x5 different samples of size 2) P = choose(a,b)*(success)^#of successes*(failure)^#of failures
...
Dont forget to subtract from 1 to get the upper tail.  The following is $$P(B \geq 5)=1-P(B < 5)$$
